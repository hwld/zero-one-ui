...
Default.test("フィルターを設定すると、ページが1に設定される", async ({ canvasElement }) => {
  ...
  await userEvent.click(await canvas.findByRole("menuitem", { name: "Todo" }));
  await userEvent.click(await canvas.findByRole("menuitem", { name: "Done" }));
  await userEvent.click(await canvas.findByRole("menuitem", { name: "未選択" }));
...
Default.test("フィルターを解除すると、ページが1に設定される", async ({ canvasElement }) => {
  const canvas = within(canvasElement.parentElement!);
  const filterOpenButton = await canvas.findByRole("button", {
    name: "絞り込み",
  });
  ...
  await userEvent.click(filterOpenButton);
  await userEvent.click(await canvas.findByRole("menuitem", { name: "Todo" }));
  await userEvent.click(await canvas.findByRole("menuitem", { name: "絞り込みを解除する" }));
