...
export const NoSelect = meta.story({
  decorators: [
    (Story) => {
      return (
        <ScrollableRootProvider>
          <MockTaskTableProvider mockSelection={{ ...mockContext, selectedTaskIds: [] }}>
            <Story />
          </MockTaskTableProvider>
        </ScrollableRootProvider>
      );
...
Default.test("タスクが選択されているときはメニューが表示される", async ({ canvasElement }) => {
...
Default.test("選択したすべてのタスクを選択解除できる", async ({ canvasElement }) => {
  const canvas = within(canvasElement.parentElement!);
...
Default.test("選択すたすべてのタスクを削除できる", async ({ canvasElement }) => {
  const canvas = within(canvasElement.parentElement!);
  ...
  await waitFor(async () => {
    ...
    await expect(mockDeleteTasks).toHaveBeenCalledWith(mockContext.selectedTaskIds);
  });
...
Default.test("選択したすべてのタスクを完了状態にできる", async ({ canvasElement }) => {
  const canvas = within(canvasElement.parentElement!);
...
NoSelect.test("何も選択されていないときにはメニューは表示されない", async ({ canvasElement }) => {
  const canvas = within(canvasElement.parentElement!);
