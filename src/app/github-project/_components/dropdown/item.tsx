...
import { ComponentPropsWithoutRef, PropsWithChildren, ReactNode, forwardRef } from "react";
...
export const DropdownItemGroup: React.FC<DropdownItemGroupProps> = ({ group, children }) => {
  return (
...
export const DropdownItemList: React.FC<DropdownItemListProps> = ({ children }) => {
...
export const DropdownItemBase = forwardRef<HTMLButtonElement, DropdownItemBaseProps>(
...
export const DropdownItem = forwardRef<HTMLButtonElement, DropdownItemProps>(function DropdownItem(
  { icon, label, red, rightIcon, ...props },
  ref,
) {
  return (
    <DropdownItemBase ref={ref} {...props} red={red}>
      <ListButtonContent red={red} icon={icon} label={label} rightIcon={rightIcon} />
    </DropdownItemBase>
  );
});
